 ...
 * Date:        2026-04-23
 */
import { MFDB_VALIDATION_CODES as E, MFDB_CORE_CODES, } from "./bejson_types";
...
// Discovery algorithm
// ---------------------------------------------------------------------------
export function discoverRole(doc, filename) {
...
// ---------------------------------------------------------------------------
export function validateManifest(doc, options = {}) {
    const r = _makeResult();
    ...
    if (typeof manifest["MFDB_Version"] !== "string" || manifest["MFDB_Version"].trim() === "") {
        _emitError(r, MFDB_CORE_CODES.INVALID_MFDB_VERSION, "Manifest is missing required header MFDB_Version.", "MFDB_Version");
    }
    if (typeof manifest["DB_Name"] !== "string" || manifest["DB_Name"].trim() === "") {
        _emitError(r, MFDB_CORE_CODES.MISSING_DB_NAME, "Manifest is missing required header DB_Name.", "DB_Name");
    }
    ...
    for (let i = 0; i < manifest.Values.length; i++) {
        ...
        // Null checks
        if (entityName === null) {
            _emitError(r, E.NULL_IN_REQUIRED_MANIFEST_FIELD, "Values[" + i + "].entity_name must not be null.", "entity_name", i);
        }
        if (filePath === null) {
            _emitError(r, E.NULL_IN_REQUIRED_MANIFEST_FIELD, "Values[" + i + "].file_path must not be null.", "file_path", i);
        }
        // Uniqueness
        if (typeof entityName === "string") {
            if (seenNames.has(entityName)) {
                _emitError(r, E.DUPLICATE_ENTRY, "Duplicate entity_name: \"" + entityName + "\".", "entity_name", i);
            }
            else {
        ...
        if (typeof filePath === "string") {
            if (seenPaths.has(filePath)) {
                _emitError(r, E.DUPLICATE_ENTRY, "Duplicate file_path: \"" + filePath + "\".", "file_path", i);
            }
            else {
            ...
            if (options.resolvedPaths && !options.resolvedPaths.has(filePath)) {
                _emitError(r, E.ENTITY_FILE_NOT_FOUND, "file_path \"" + filePath + "\" does not exist on disk.", "file_path", i);
            }
        }
    }
    return r;
}
export function validateEntityFile(doc, options) {
    const r = _makeResult();
    ...
    if (actualName !== options.expectedEntityName) {
        _emitError(r, E.ENTITY_NAME_MISMATCH, "Entity file Records_Type[0] is \"" + actualName + "\" but manifest expects \"" + options.expectedEntityName + "\".", "Records_Type");
    }
    ...
    if (options.expectedParentHierarchy !== undefined &&
        ...
        _emitError(r, E.MANIFEST_FILE_NOT_FOUND, "Parent_Hierarchy \"" + entity.Parent_Hierarchy + "\" does not match expected \"" + options.expectedParentHierarchy + "\".", "Parent_Hierarchy");
    }
    ...
    if (options.entityRelativePath !== undefined && options.manifestRelativePath !== undefined) {
        if (typeof entity.Parent_Hierarchy === "string") {
            _emitWarning(r, E.BIDIRECTIONAL_PATH_FAILED, "Bidirectional path check: verify that \"" + options.entityRelativePath +
    ...
    if (typeof entity.Parent_Hierarchy === "string") {
        if (entity.Parent_Hierarchy.includes("..") && _escapesRoot(entity.Parent_Hierarchy)) {
            _emitError(r, E.MISSING_PARENT_HIERARCHY, "Parent_Hierarchy must not escape the database root directory.", "Parent_Hierarchy");
        }
    }
    return r;
}
export function validateDatabase(manifest, entityDocs, options = {}) {
    const r = _makeResult();
    // Level 1
    const l1 = validateManifest(manifest, { resolvedPaths: options.resolvedPaths });
    for (const e of l1.errors)
        _emitError(r, e.code, "[L1] " + e.message, e.field, e.recordIndex);
    for (const w of l1.warnings)
        _emitWarning(r, w.code, "[L1] " + w.message, w.field, w.recordIndex);
    if (!r.valid)
        return r;
    ...
    for (const record of records) {
        ...
        if (!entityDoc) {
            _emitError(r, E.ENTITY_FILE_NOT_FOUND, "[L2] Entity document not provided for file_path \"" + record.file_path + "\".", "file_path");
            continue;
        }
        ...
        for (const e of l2.errors)
            _emitError(r, e.code, "[L2:" + record.entity_name + "] " + e.message, e.field, e.recordIndex);
        for (const w of l2.warnings)
            _emitWarning(r, w.code, "[L2:" + record.entity_name + "] " + w.message, w.field, w.recordIndex);
    }
...
// ---------------------------------------------------------------------------
export function decodeManifestRecords(manifest) {
    const fieldNames = manifest.Fields.map((f) => f.name);
...
export function decodeDatabaseMeta(manifest) {
    return {
...
function _checkRecordCounts(manifestDoc, records, entityDocs, r) {
    ...
    for (let i = 0; i < records.length; i++) {
        ...
        if (actualCount !== record.record_count) {
            _emitWarning(r, 0, "[L3] record_count mismatch for \"" + record.entity_name + "\": manifest says " +
