 ...
 */
import { BEJSONValidationError, BEJSON_VALIDATION_CODES as E, } from "./bejson_types";
// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------
...
export function validate104(doc) {
...
export function validate104a(doc) {
    const result = validateDocument(doc);
    if (result.valid) {
        const bej = doc;
...
export function validate104db(doc) {
    const result = validateDocument(doc);
    if (result.valid) {
        const bej = doc;
...
function _checkValues(doc, r) {
    ...
    for (let i = 0; i < doc.Values.length; i++) {
        ...
        if (row.length !== fieldCount) {
            _err(r, E.RECORD_LENGTH_MISMATCH, "Values[" + i + "] has " + row.length + " elements but Fields has " + fieldCount + ".", undefined, i);
            continue;
        }
        ...
        for (let fi = 0; fi < fieldCount; fi++) {
            ...
            if (!_typeMatches(val, field.type)) {
                _err(r, E.VALUE_TYPE_MISMATCH, "Values[" + i + "][" + fi + "] (" + field.name + "): expected " + field.type + ", got " + typeof val + ".", field.name, i);
            }
        }
    }
}
...
function _check104aSpecific(doc, r) {
    ...
    for (let i = 0; i < doc.Fields.length; i++) {
        ...
        if (!PRIMITIVE_TYPES.includes(field.type)) {
            _err(r, E.VERSION_CONSTRAINT, "BEJSON 104a: Fields[" + i + "].type \"" + field.type + "\" is not a primitive type.", field.name);
        }
    }
    ...
    for (const key of Object.keys(doc)) {
        ...
        if (!PASCAL_CASE_RE.test(key)) {
            _err(r, E.INVALID_CUSTOM_KEY, "BEJSON 104a: custom header \"" + key + "\" must be PascalCase.", key);
        }
    }
}
// ---------------------------------------------------------------------------
...
function _check104dbSpecific(doc, r) {
    ...
    for (let i = 1; i < doc.Fields.length; i++) {
        ...
        if (!field.Record_Type_Parent) {
            _err(r, E.MISSING_DISCRIMINATOR, "BEJSON 104db: Fields[" + i + "] (\"" + field.name + "\") is missing Record_Type_Parent assignment.", field.name);
        }
        else if (!validEntities.has(field.Record_Type_Parent)) {
            _err(r, E.VERSION_CONSTRAINT, "BEJSON 104db: Fields[" + i + "].Record_Type_Parent \"" + field.Record_Type_Parent + "\" is not in Records_Type.", field.name);
        }
    }
    ...
    if (Array.isArray(doc.Values)) {
        for (let i = 0; i < doc.Values.length; i++) {
            ...
            if (typeof discriminator !== "string" || !validEntities.has(discriminator)) {
                _err(r, E.VERSION_CONSTRAINT, "Values[" + i + "][0] discriminator \"" + discriminator + "\" not in Records_Type.", "Record_Type_Parent", i);
            }
        }
    }
}
